...
| `components/Button.jsx` | **Reusable button component.** Variants: `primary` (indigo filled), `secondary` (dark filled), `outline` (gray bordered), `indigo-outline` (indigo bordered), `ghost` (transparent hover). Sizes: `sm` (`h-9 px-3.5`), `md` (`h-12 px-5`, default), `lg` (`h-14 px-7`). Props: `variant`, `size`, `loading` (shows spinner), `fullWidth`, `icon` (before children), `iconAfter` (after children). Sized CTA buttons are **pill-shaped** (`rounded-full`); `size="raw"` custom buttons (cards, list rows) keep a `rounded-2xl` fallback. All CTA buttons across the app use this component. |
